...
- **Auto-detection** — automatically finds all humidity sensors in your Home Assistant instance (device_class: humidity with % unit); no manual entity ID typing needed
- **Sensor overview popup** — tap the pill to open a popup showing each sensor as its own pill with the current reading, plus a stats bar for low, average and high across all sensors
- **Time-range history graph** — tap any sensor pill to open a graph popup with a 1h / 3h / 6h / 12h / 24h segmented time selector and a smooth SVG line graph
